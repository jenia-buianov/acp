...
function pointsDateToLine($date){
    $explode = explode('.',$date);
    return '20'.$explode[2].'-'.$explode[1].'-'.$explode[0];
}

function textToDate($string){
    $explode = explode(' ',$string);
    $day = (int)$explode[0];
    $yaer = (int)$explode[2];
    $months = array('Январь','Ферваль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь');
    $month = (int)array_search($explode[1],$months)+1;
...
if (!function_exists('isAjax')){
    ...
    function isAjax(){
        if(isset($_POST)and!empty($_POST)and !empty($_POST['_token'])) return true;
        return false;
    }
}
...
if (!function_exists('translate')){
    ...
    function translate($key){
        ...
        return htmlspecialchars_decode($result->text);
